...
const Header = ({themeTogglerRef}) => {
    ...
    return (
        <header 
          className="grid grid-cols-4 bg-cyan-100 shadow-sm shadow-cyan-200 
          justify-items-strech justify-self-start w-screen h-14 py-4 fixed top-0 opacity-90"
        >
            ...
            <div 
              className="col-span-2 flex flex-row justify-self-end space-x-1.5 
                ml-auto mt-0 md:space-x-3 lg:space-x-4 xl:space-x-9 xl:text-lg 2xl:space-x-16"
            >
                <a href="/" className="flex flex-row rounded-lg text-center text-smd text-cyan-600 ml-7 hover:text-blue-300 
                  font-semibold italic md:text-sm lg:flex-row lg:text-md xl:flex-row xl:text-lg 2xl:flex-row 2xl:text-lg"
                >
            ...
            <button 
                className="flex ml-8 p-1 rounded-sm ring-2 ring-wisteria bg-zinc-600 
                 text-cyan-50 w-28 h-8 hover:bg-cyan-50 hover:text-zinc-700 hover:ring-french-rose" 
                ref={themeTogglerRef}
            >
