...
const Desktop = () => {
    ...
    const initialPageContainerCls = "grid-rows-24 w-auto h-screen space-y-0 overflow-x-hidden";
    const initialPageToggleThemeBtnCls =
        "grid grid-cols-2 rounded-md w-28 fixed top-16 opacity-75 mr-2 justify-self-end bg-zinc-600 text-maximum-green-yellow ring-2 " + 
        "ring-wisteria justify-items-center px-1 py-1 transition ease-in-out delay-150 hover:-translate-y-1 " + 
        "hover:scale-110 hover:bg-zinc-500 duration-700 hover:skew-y-6";
    ...
    const [sideMenuClass, setSideMenuClass] = useState("grid grid-row-7 rounded-r-lg bg-cyan-900 shadow-sm shadow-slate-200 py-2 visible");
    const [productoAttrOpt, setProductoAttrOpt] = useState('');
    ...
    function handPageThemeToggler() {
        ...
        (
            ...
            "grid grid-cols-2 w-28 ml-9 rounded-md fixed top-16 opacity-75 mr-2 justify-self-end bg-white " + 
            "text-zinc-700 ring-2 ring-wisteria justify-items-center px-3 py-1 transition ease-in delay-300 " + 
            "hover:-translate-y-1 hover:scale-110 hover:bg-zinc-100 hover:text-french-rose duration-200 hover:skew-y-6",
            mainBoxesCls += " bg-zinc-500 text-cyan-50 mt-8"
        );

        contextApi.screenThemeToggler = auxTogglerValidation;
    ...
    useEffect(() => {
        formRef.current.addEventListener("submit", ev => {
            ...
            for(let d of data) {
                if (d[1]) {
                    setProductoAttrOpt(d[0]);
                } 
            }

            console.log(productoAttrOpt, " was selected!")
        });
        ...
        !themeTogglerAction ? (
            ...
            mutableDateTimeClass.current = INIT_DATE_TIME_STR + " text-maximum-green-yellow",
            mutableInitialPageTgThemeBtnCls.current =
            "grid grid-cols-2 w-28 ml-9 rounded-md fixed top-16 opacity-75 mr-2 justify-self-end " + 
            "bg-white text-zinc-700 ring-2 ring-wisteria justify-items-center px-3 py-1 transition " + 
            "ease-in delay-300 hover:-translate-y-1 hover:scale-110 " + 
            "hover:bg-zinc-100 hover:text-french-rose duration-200 hover:skew-y-6"
        );
    ...
    useEffect(() => {
        ...
        burguerMenuBtnClickSt ? (
            ...
            setSideMenuClass(
                "grid grid-rows-2 rounded-r-md bg-cyan-900 shadow-sm shadow-slate-200 mb-0 py-4 space-y-5 h-11/12 custom-css-side-menu md:visible lg:visible xl:visible 2xl:visible"
            )
        );
    }, [burguerMenuBtnClickSt]);
    ...
    return (
        <div className={mutablePageContainerCls.current}>
            ...
            <div className="grid grid-cols-12 rounded-md ring-1 ring-black z-10 
                admin-background-img shadow-sm shadow-slate-200 mx-0 mt-1 py-5 md:mx-2 lg:mx-2 xl:mx-2 2xl:mx-2"
            >
                <h3 className="col-span-7 mr-7 mt-4 pt-0 h-5 text-end text-2xl text-cyan-50 px-auto font-bold ml-2 
                  py-8 md:text-3xl md:ml-auto lg:text-4xl lg:ml-auto xl:text-4xl xl:ml-auto 2xl:text-4xl 2xl:ml-auto"
                >
                    Administrador
                </h3>
                <ul className="col-span-5 rounded-sm bg-cyan-50 px-4 py-2 space-y-2 text-lg text-french-rose">
                    <li>
                        <span className="flex flex-row space-x-0.5">
                            ...
                            <small className="bg-yellow-200 px-1 py-0 text-sm 
                              font-semibold rounded-md ring-1 ring-french-rose"
                            >
                                8
                            </small>
                        </span>
            ...
            <div 
                className="row-span-8 grid grid-cols-12 px-0 py-2 md:mr-4 md:px-1 
                  lg:mr-12 lg:px-1 xl:mr-11 xl:px-1 2xl:mr-4 2xl:px-2"
            >
                <div className="col-span-12 grid grid-rows-8 flex-col rounded-md py-2 h-full text-center 
                space-y-7 md:col-span-4 md:visible lg:col-span-3 xl:col-span-2 2xl:col-span-4"
                >
                    <div className="flex flex-row px-2">
                        <button 
                            className="rounded-xl ml-2" 
                            id="burguer_admin_btn" 
                            ref={menuBarBurguerClickBtnRef}
                        >
                        ...
                        <p className="text-md text-zinc-500 ml-auto md:ml-0 md:mx-auto md:text-lg 
                        lg:ml-0 lg:mx-auto lg:text-lg xl:ml-0 xl:mx-auto xl:text-lg 2xl:ml-0 2xl:mx-auto 2xl:text-lg"
                        >
                            Hola, <span className="italic">{window.localStorage.getItem("email").split('@').at(0)}</span>
                        </p>
                    </div>

                    <div className={sideMenuClass}>
                        <h5 className="font-semibold h-6 rounded-md text-apple-green mx-auto">Menú</h5>
                        <form className="flex flex-col space-y-0.5 px-1" ref={formRef}>
                            ...
                            <div className="flex space-x-0.5 self-center w-44 md:mx-0 lg:mx-0 xl:mx-0 2xl:mx-0">
                                <input 
                                    ...
                                    className="rounded-l-2xl h-8 w-40
                                ...
                                <button type="submit" className="rounded-r-2xl text-3xl text-white w-8 h-8
                                  cursor-pointer ring-2 ring-french-rose hover:text-zinc-200 
                        ...
                        <ul className="flex flex-col space-y-9 py-2">
                            <li 
                                className="flex space-x-0.5 mx-auto text-white cursor-pointer hover:text-maximum-green-yellow" 
                                onClick={() => {
                                    !actionShowedPList ? (
                                        setActionShowedList(true), 
                                        auxProds = manageProductsData(), 
                                        setProductos(auxProds)
                                    ) : setActionShowedList(false)
                                }
                            }>
                                Listar/Ocultar productos
                                <ImCircleDown className="mx-auto ml-1 mt-1 text-md" />
                            </li>
                            ...
                            <li className="text-white cursor-pointer hover:text-maximum-green-yellow">
                                ...
                                >
                                    Agregar inventario 
                                    <button className="bg-cyan-500 rounded-md"><BiPlus className="text-xl" /></button>
                                </Link>
                            </li>
                            ...
                            <li className="cursor-pointer hover:text-maximum-green-yellow">
                                ...
                                <a
                                    href="#add_prod_modal" 
                                    rel="modal:open" 
                                    className="text-white hover:text-maximum-green-yellow mx-auto space-x-1"
                                >
                ...
                <div className="row-span-3 col-start-1 col-span-10 ml-8 mt-14 pl-6 py-0 w-auto space-y-3.5 text-xl 
                        ...
                        !actionShowedPList ?
                            (
                                <div className={mainBoxesCls}>
                                    <h4>Productos</h4>
                                    <div className="flex justify-center space-x-4 md:space-x-9 lg:space-x-11 xl:space-x-11 2xl:space-x-11">
                                        <button 
                                            className="rounded-3xl bg-zinc-600 ring-1 ring-black text-sm text-white
                                            justify-self-center mb-2 w-12 h-12 scale-125 hover:bg-zinc-500 hover:text-cyan-50 hover:scale-150" 
                                            onClick={() => {
                                                !actionShowedPList ? (
                                                    setActionShowedList(true), 
                                                    auxProds = manageProductsData(), 
                                        ...
                                        <small className="mt-3">o</small>

                                        <button className="rounded-3xl bg-red-500 ring-1 ring-black text-sm text-white 
                                        justify-self-center w-12 h-12 scale-125 hover:bg-red-400 hover:text-cyan-900 hover:scale-150"
                                        >
                                            Borrar
                                        </button>

                                        <small className="mt-3">todos</small>
                                    </div>
                                </div>
                                            ...
                                            <>
                                                <div className={mainBoxesCls}>
                                                    ...
                                                    <div className="grid place-items-center">
                                                        <button 
                                                            className="rounded-sm bg-air-super-blue ring-1 ring-black text-sm text-white 
                                                              justify-self-center mb-2 px-9 hover:bg-middle-blue hover:text-cyan-900 hover:skew-x-2" 
                                                            onClick={() => {
                                                                !actionShowedPList ? (
                                                                    setActionShowedList(true), 
                                                                    auxProds = manageProductsData(), 
